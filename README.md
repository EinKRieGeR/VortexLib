...
1. **Управление состоянием**: Текущий прогресс импорта и кэши хранятся в абстрактном слое Nitro. Это позволяет переключаться между памятью, Redis или файловым драйвером (для сохранения состояния между перезапусками хоста) в `nuxt.config.ts`:
  ```ts
  nitro: {
    storage: { 
      memory: { 
